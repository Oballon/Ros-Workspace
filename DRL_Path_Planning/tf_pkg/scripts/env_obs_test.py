...
class Env:
    ...
    # 重置environment
    def reset_env(self, start=[0.0, 0.0], goal=[10.0, 10.0], Randangle=0.0, OBS_X=np.zeros(10), OBS_Y=np.zeros(10)):
        self.sp = start
        self.gp = goal
        # 初始点到目标点的距离
        ...
        for i in range(self.num_obs):
            self.obs_pos.append([])
            for j in range(2):
                self.obs_pos[i].append(0.0)

        for i in range(self.num_obs):
            self.obs_pos[i][0] = OBS_X[i]
            self.obs_pos[i][1] = OBS_Y[i]

        # print("self.obs_pos={}".format(self.obs_pos))
