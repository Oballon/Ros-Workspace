...
# 环境模型：gazebo_env_D3QN_PER_image_add_sensor_empty_world_30m_test.py
...
class DQN:
    ...
    def main(self):
        ...
        env.reset_env(start=[start_x[0], start_y[0]], goal=[goal_x[0], goal_y[0]], Randangle=randangle[0])
        ...
        while True:
            ...
            if terminal == True:
                ...
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/test_empty_world_30m_results/image_add_sensor/第二次训练/10_D3QN_PER_image_add_sensor_empty_world_30m_test_JACKAL_PATH_x' + '_' + str(
                        i) + '.txt', JACKAL_x, delimiter=',')
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/test_empty_world_30m_results/image_add_sensor/第二次训练/10_D3QN_PER_image_add_sensor_empty_world_30m_test_JACKAL_PATH_y' + '_' + str(
                        i) + '.txt', JACKAL_y, delimiter=',')
                jackal_x = []
                jackal_y = []
                ...
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/test_empty_world_30m_results/image_add_sensor/第二次训练/reward_pic/10_D3QN_PER_image_add_sensor_empty_world_30m_reward_test.txt',
                    reward_array, delimiter=',')
                ...
                # 随机初始化起点和终点的位置
                env.reset_env(start=[start_x[i], start_y[i]], goal=[goal_x[i], goal_y[i]], Randangle=randangle[i])
                # print("start_x={},start_y={},goal_x={},goal_y={},d_sp={}".format(start_x[i],start_y[i],goal_x[i],goal_y[i],math.sqrt((start_x[i]-goal_x[i])**2+(start_y[i]-goal_y[i])**2)))
                env_info, jackal_x_temp, jackal_y_temp = env.get_env()
                jackal_x.append(jackal_x_temp)
                jackal_y.append(jackal_y_temp)
                self.observation_stack, self.observation_set, self.state_stack, self.state_set = self.input_initialization(
