...
class DQN:
    ...
    def main(self):
        ...
        env.reset_env(start=[start_x[0], start_y[0]], goal=[goal_x[0], goal_y[0]], Randangle=randangle[0],
                      OBS_X=obs_pos_x[0], OBS_Y=obs_pos_y[0])
        ...
        while True:
            ...
            if terminal == True:
                ...
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/10_D3QN_PER_image_add_sensor_obstacle_world_30m_test_JACKAL_PATH_x' + '_' + str(
                        i) + '.txt', JACKAL_x, delimiter=',')
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/10_D3QN_PER_image_add_sensor_obstacle_world_30m_test_JACKAL_PATH_y' + '_' + str(
                ...
                # 第一次训练
                np.savetxt(
                    '.../DRL_Path_Planning/src/tf_pkg/scripts/10_D3QN_PER_image_add_sensor_obstacle_world_30m_reward_test.txt',
                ...
                env.reset_env(start=[start_x[i], start_y[i]], goal=[goal_x[i], goal_y[i]], Randangle=randangle[i],
                              OBS_X=obs_pos_x[i], OBS_Y=obs_pos_y[i])
                env_info, jackal_x_temp, jackal_y_temp = env.get_env()
                jackal_x.append(jackal_x_temp)
                jackal_y.append(jackal_y_temp)
                self.observation_stack, self.observation_set, self.state_stack, self.state_set = self.input_initialization(
